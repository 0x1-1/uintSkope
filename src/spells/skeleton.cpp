	...
	bool doShape( NifModel * nif, const QModelIndex & index, const Transform & tparent, const TransMap & world, const TransMap & bones )
	{
		...
		if ( iNames.isValid() )
			for ( int n = 0; n < nif->rowCount( iNames ); n++ ) {
				QModelIndex iBone = nif->getBlock( nif->getLink( getChildIndex(iNames,  n, 0 ) ), "NiNode" );
		...
		for ( int b = 0; b < nif->rowCount( iBones ) && b < names.count(); b++ ) {
			QModelIndex iBone = getChildIndex(iBones,  b, 0 );
...
// Qt 6: qMapLessThanKey<> is no longer a customization point; QMap<Triangle, ...>
// now orders via Triangle::operator< (defined in niftypes.h).
	...
	QModelIndex cast( NifModel * nif, const QModelIndex & iBlock, int & maxBonesPerPartition, int & maxBonesPerVertex, bool make_strips, bool pad = false )
	{
		...
		try
		{
			...
			for ( int bone = 0; bone < numBones; bone++ ) {
				QModelIndex iVertexWeights = nif->getIndex( getChildIndex(iBoneList,  bone, 0 ), "Vertex Weights" );

				for ( int r = 0; r < nif->rowCount( iVertexWeights ); r++ ) {
					int vertex = nif->get<int>( getChildIndex(iVertexWeights,  r, 0 ), "Index" );
					float weight = nif->get<float>( getChildIndex(iVertexWeights,  r, 0 ), "Weight" );
			...
			if ( iShapeType == "NiTriShape" ) {
				...
				for ( int s = 0; s < nif->rowCount( iPoints ); s++ ) {
					...
					QModelIndex iStrip = getChildIndex(iPoints,  s, 0 );

					for ( int p = 0; p < nif->rowCount( iStrip ); p++ ) {
						strip.append( nif->get<int>( getChildIndex(iStrip,  p, 0 ) ) );
					}
			...
			if ( nif->inherits( iSkinInst, "BSDismemberSkinInstance" ) ) {
				...
				for ( quint32 i = 0; i < nparts; ++i ) {
					QModelIndex iPart = getChildIndex(iPartData,  i, 0 );
			...
			for ( int p = 0; p < parts.count(); p++ ) {
				QModelIndex iPart = getChildIndex( nif->getIndex( iSkinPart, "Skin Partition Blocks" ), p, 0 );
				...
				if ( iBSSkinInstPartData.isValid() ) {
					if ( bones != prevPartBones ) {
						prevPartBones = bones;
						nif->set<uint>( getChildIndex(iBSSkinInstPartData,  p, 0 ), "Part Flag", 257 );
					}
				}
				...
				for ( int v = 0; v < nif->rowCount( iVWeights ); v++ ) {
					QModelIndex iVertex = getChildIndex(iVWeights,  v, 0 );
					nif->updateArray( iVertex );
					QList<boneweight> list = weights.value( vertices[v] );

					for ( int b = 0; b < maxBones; b++ )
						nif->set<float>( getChildIndex(iVertex,  b, 0 ), list.count() > b ? list[ b ].second : 0.0 );
				}
				...
				if ( make_strips == true ) {
					...
					for ( int s = 0; s < nif->rowCount( iStripLengths ); s++ )
						nif->set<int>( getChildIndex(iStripLengths,  s, 0 ), strips.value( s ).count() );

					QModelIndex iStrips = nif->getIndex( iPart, "Strips" );
					nif->updateArray( iStrips );

					for ( int s = 0; s < nif->rowCount( iStrips ); s++ ) {
						nif->updateArray( getChildIndex(iStrips,  s, 0 ) );
						nif->setArray<quint16>( getChildIndex(iStrips,  s, 0 ), strips.value( s ) );
					}
				} else {
				...
				for ( int v = 0; v < nif->rowCount( iVBones ); v++ ) {
					QModelIndex iVertex = getChildIndex(iVBones,  v, 0 );
					nif->updateArray( iVertex );
					QList<boneweight> list = weights.value( vertices[v] );

					for ( int b = 0; b < maxBones; b++ )
						nif->set<int>( getChildIndex(iVertex,  b, 0 ), list.count() > b ? bones.indexOf( list[ b ].first ) : 0 );
				}
			}
...
public:
	...
	QModelIndex cast( NifModel * nif, const QModelIndex & iSkinData ) override final
	{
		...
		for ( int n = 0; n < nif->rowCount( iBoneMap ); n++ ) {
			QModelIndex iBone = nif->getBlock( nif->getLink( getChildIndex(iBoneMap,  n, 0 ) ), "NiNode" );
		...
		for ( int b = 0; b < nif->rowCount( iBoneDataList ); b++ ) {
			...
			QModelIndex iWeightList = nif->getIndex( getChildIndex(iBoneDataList,  b, 0 ), "Vertex Weights" );

			for ( int w = 0; w < nif->rowCount( iWeightList ); w++ ) {
				int v = nif->get<int>( getChildIndex(iWeightList,  w, 0 ), "Index" );
			...
			auto sphIdx = nif->getIndex( getChildIndex(iBoneDataList,  b, 0 ) , "Bounding Sphere" );
		...
		if ( iTransKeys.isValid() ) {
			...
			if ( iTransKeys.isValid() ) {
				for ( int k = 0; k < nif->rowCount( iTransKeys ); k++ ) {
					QModelIndex iKey = getChildIndex(iTransKeys,  k, 0 );
