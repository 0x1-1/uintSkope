		...
		for ( int n = 0; n < nif->getBlockCount(); n++ ) {
			...
			if ( iNumChildren.isValid() && iChildren.isValid() ) {
				...
				for ( int r = 0; r < links.count(); r++ ) {
					if ( links[r].first != nif->getLink( getChildIndex(iChildren,  r, 0 ) ) ) {
						nif->setLink( getChildIndex(iChildren,  r, 0 ), links[r].first );
					}
				}
	...
	QModelIndex cast( NifModel * nif, const QModelIndex & ) override final
	{
		...
		for ( int i = 0; i < nif->getBlockCount(); i++ ) {
			...
			for ( int i = 0; i < numBlocks; i++ ) {
				auto ctrlrType =  nif->getIndex( getChildIndex(controlledBlocks,  i, 0 ), "Controller Type" );
				auto nodeName = nif->getIndex( getChildIndex(controlledBlocks,  i, 0 ), "Node Name" );
