...
QModelIndex spTangentSpace::cast( NifModel * nif, const QModelIndex & iBlock )
{
	...
	if ( nif->getUserVersion2() < 100 ) {
		...
		iTexCo = getChildIndex(iTexCo,  0, 0 );
	...
	if ( iPoints.isValid() ) {
		...
		if ( iPartBlock.isValid() ) {
			...
			for ( int i = 0; i < numParts; i++ )
				triangles << nif->getArray<Triangle>( getChildIndex(iParts,  i, 0 ), "Triangles" );
		} else {
	...
	if ( isOblivion ) {
		...
		if ( !iTSpace.isValid() ) {
			...
			if ( iNumExtras.isValid() && iExtras.isValid() ) {
				...
				nif->setLink( getChildIndex(iExtras,  numlinks, 0 ), nif->getBlockNumber( iTSpace ) );
			}
		}
