...
class spStrippify final : public Spell
{
	...
	QModelIndex cast( NifModel * nif, const QModelIndex & index ) override final
	{
		...
		for ( int t = 0; t < nif->rowCount( iTriangles ); t++ ) {
			Triangle tri = nif->get<Triangle>( getChildIndex(iTriangles,  t, 0 ) );
		...
		if ( iStripData.isValid() ) {
			...
			if ( iDstUV.isValid() && iSrcUV.isValid() ) {
				...
				for ( int r = 0; r < nif->rowCount( iDstUV ); r++ ) {
					copyArray<Vector2>( nif, getChildIndex(iDstUV,  r, 0 ), getChildIndex(iSrcUV,  r, 0 ) );
				}
			}
			...
			if ( iLengths.isValid() && iPoints.isValid() ) {
				...
				int x = 0;
				for ( const QVector<quint16>& strip : strips ) {
					nif->set<int>( getChildIndex(iLengths,  x, 0 ), strip.count() );
					QModelIndex iStrip = getChildIndex(iPoints,  x, 0 );
		...
		if ( numTriangles > USHRT_MAX ) {
			...
			if ( iLengths.isValid() && iPoints.isValid() ) {
				nif->updateArray( iLengths );
				nif->set<quint16>( getChildIndex(iLengths,  0, 0 ), stripsA.count() );
				nif->updateArray( iPoints );
				nif->updateArray( getChildIndex(iPoints,  0, 0 ) );
				nif->setArray<quint16>( getChildIndex(iPoints,  0, 0 ), stripsA );
			...
			if ( iLengths.isValid() && iPoints.isValid() ) {
				nif->updateArray( iLengths );
				nif->set<quint16>( getChildIndex(iLengths,  0, 0 ), stripsB.count() );
				nif->updateArray( iPoints );
				nif->updateArray( getChildIndex(iPoints,  0, 0 ) );
				nif->setArray<quint16>( getChildIndex(iPoints,  0, 0 ), stripsB );
...
public:
	...
	QModelIndex cast( NifModel * nif, const QModelIndex & index ) override final
	{
		...
		nif->set<int>( getChildIndex(iLength,  0, 0 ), strip.size() );
		nif->updateArray( iPoints );
		nif->updateArray( getChildIndex(iPoints,  0, 0 ) );
		nif->setArray<quint16>( getChildIndex(iPoints,  0, 0 ), strip );
...
public:
	...
	QModelIndex cast( NifModel * nif, const QModelIndex & index ) override final
	{
		...
		QVector<quint16> strip = nif->getArray<quint16>( getChildIndex(iPoints,  0, 0 ) );
		...
		for ( int r = 0; r < strips.count(); r++ ) {
			nif->set<int>( getChildIndex(iLength,  r, 0 ), strips[r].size() );
			nif->updateArray( getChildIndex(iPoints,  r, 0 ) );
