	...
	QModelIndex cast( NifModel * nif, const QModelIndex & index ) override final
	{
		...
		if ( act ) {
			...
			if ( selFrame == 0 ) {
				...
				nif->updateArray( getChildIndex(iFrames,  0, 0 ), "Vectors" );
				nif->setArray( getChildIndex(iFrames,  0, 0 ), "Vectors", verts );
				verts.fill( Vector3() );

				for ( int f = 1; f < nif->rowCount( iFrames ); f++ ) {
					nif->updateArray( getChildIndex(iFrames,  f, 0 ), "Vectors" );
					nif->setArray<Vector3>( getChildIndex(iFrames,  f, 0 ), "Vectors", verts );
				}
			} else {
				QVector<Vector3> verts = nif->getArray<Vector3>( iMeshData, "Vertices" );
				QVector<Vector3> base  = nif->getArray<Vector3>( getChildIndex(iFrames,  0, 0 ), "Vectors" );
				QVector<Vector3> frame( base.count(), Vector3() );

				for ( int n = 0; n < base.count(); n++ )
					frame[ n ] = verts.value( n ) - base[ n ];

				nif->setArray<Vector3>( getChildIndex(iFrames,  selFrame, 0 ), "Vectors", frame );
			}
		}
	...
	QStringList listFrames( const NifModel * nif, const QModelIndex & iMorpher )
	{
		...
		if ( iFrames.isValid() ) {
			...
			for ( int i = 0; i < nif->rowCount( iFrames ); i++ ) {
				list << nif->get<QString>( getChildIndex(iFrames,  i, 0 ), "Frame Name" );
			}
