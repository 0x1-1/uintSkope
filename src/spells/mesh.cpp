...
static void removeWasteVertices( NifModel * nif, const QModelIndex & iData, const QModelIndex & iShape )
{
	try
	{
		...
		for ( int r = 0; r < nif->rowCount( iUVSets ); r++ ) {
			texco << nif->getArray<Vector2>( getChildIndex(iUVSets,  r, 0 ) );
		...
		for ( int r = 0; r < nif->rowCount( iPoints ); r++ ) {
			strips << nif->getArray<quint16>( getChildIndex(iPoints,  r, 0 ) );
		...
		for ( int r = 0; r < nif->rowCount( iPoints ); r++ )
			nif->setArray<quint16>( getChildIndex(iPoints,  r, 0 ), strips[r] );
		...
		for ( int r = 0; r < nif->rowCount( iUVSets ); r++ ) {
			nif->updateArray( getChildIndex(iUVSets,  r, 0 ) );
			nif->setArray<Vector2>( getChildIndex(iUVSets,  r, 0 ), texco[r] );
		}
		...
		for ( int b = 0; b < nif->rowCount( iBones ); b++ ) {
			...
			QModelIndex iWeights = nif->getIndex( getChildIndex(iBones,  b, 0 ), "Vertex Weights" );

			for ( int w = 0; w < nif->rowCount( iWeights ); w++ ) {
				weights.append( QPair<int, float>( nif->get<int>( getChildIndex(iWeights,  w, 0 ), "Index" ), nif->get<float>( getChildIndex(iWeights,  w, 0 ), "Weight" ) ) );
			}
			...
			nif->set<int>( getChildIndex(iBones,  b, 0 ), "Num Vertices", weights.count() );
			nif->updateArray( iWeights );

			for ( int w = 0; w < weights.count(); w++ ) {
				nif->set<int>( getChildIndex(iWeights,  w, 0 ), "Index", weights[w].first );
				nif->set<float>( getChildIndex(iWeights,  w, 0 ), "Weight", weights[w].second );
			}
		}
...
public:
	...
	void flip( NifModel * nif, const QModelIndex & index, int f )
	{
		if ( nif->isArray( index ) ) {
			QModelIndex idx = getChildIndex(index,  0, 0 );
...
public:
	...
	bool isApplicable( const NifModel * nif, const QModelIndex & index ) override final
	{
		return ( nif->getValue( index ).type() == NifValue::tTriangle )
		       || ( nif->isArray( index ) && nif->getValue( getChildIndex(index,  0, 0 ) ).type() == NifValue::tTriangle );
	}
	...
	QModelIndex cast( NifModel * nif, const QModelIndex & index ) override final
	{
		try
		{
			...
			for ( int r = 0; r < nif->rowCount( iPoints ); r++ ) {
				...
				nif->setArray<quint16>( getChildIndex(iPoints,  r, 0 ), strip );
			}
...
public:
	...
	QModelIndex cast( NifModel * nif, const QModelIndex & index ) override final
	{
		...
		for ( int i = 0; i < nif->rowCount( vertData ); i++ ) {
			verts << nif->get<Vector3>( getChildIndex(vertData,  i, 0 ), "Vertex" );
		}
...
QModelIndex spUpdateTrianglesFromSkin::cast( NifModel * nif, const QModelIndex & index )
{
	...
	for ( int i = 0; i < nif->rowCount( iParts ) && iParts.isValid(); i++ )
		tris << SkinPartition( nif, getChildIndex(iParts,  i, 0 ) ).getRemappedTriangles();
