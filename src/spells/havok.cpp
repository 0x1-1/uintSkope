...
public:
	...
	QModelIndex cast( NifModel * nif, const QModelIndex & index ) override final
	{
		...
		nif->set<float>( getChildIndex( nif->getIndex( iCVS, "Unknown 6 Floats" ), 2, 0 ), -0.0 );
		nif->set<float>( getChildIndex( nif->getIndex( iCVS, "Unknown 6 Floats" ), 5, 0 ), -0.0 );
...
public:
	...
	QModelIndex cast( NifModel * nif, const QModelIndex & index ) override final
	{
		...
		QModelIndex iBodyA = nif->getBlock( nif->getLink( getChildIndex( nif->getIndex( iConstraint, "Entities" ), 0, 0 ) ), "bhkRigidBody" );
		QModelIndex iBodyB = nif->getBlock( nif->getLink( getChildIndex( nif->getIndex( iConstraint, "Entities" ), 1, 0 ) ), "bhkRigidBody" );
...
public:
	...
	QModelIndex cast( NifModel * nif, const QModelIndex & iBlock ) override final
	{
		...
		for ( const auto lData : nif->getLinkArray( iShape, "Strips Data" ) ) {
			...
			if ( iData.isValid() ) {
				...
				for ( int x = 0; x < nif->rowCount( iPoints ); x++ ) {
					tris += triangulate( nif->getArray<quint16>( getChildIndex(iPoints,  x, 0 ) ) );
				}
		...
		nif->set<int>( getChildIndex(iSubShapes,  0, 0 ), "Layer", 1 );
		nif->set<int>( getChildIndex(iSubShapes,  0, 0 ), "Num Vertices", vertices.count() );
		nif->set<int>( getChildIndex(iSubShapes,  0, 0 ), "Material", nif->get<int>( iShape, "Material" ) );
		...
		for ( int t = 0; t < triangles.size(); t++ ) {
			nif->set<Triangle>( getChildIndex(iTriangles,  t, 0 ), "Triangle", triangles[ t ] );
			nif->set<Vector3>( getChildIndex(iTriangles,  t, 0 ), "Normal", normals.value( t ) );
		}
